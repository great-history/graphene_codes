function [H_t_K, H_t_Kp] = construct_trilayer_effective_Ham_with_D(gamma0, gamma1, gamma2, gamma3, gamma4, gamma5, delta, Delta1, Delta2, akx, aky)
    H_t_K = zeros(6,6);
    H_t_Kp = zeros(6,6);
    ...
    H_t_Kp(1:2,1:2) = H_m_Kp;
    H_t_Kp(3:6,3:6) = H_b_Kp;
    ...
    hem2 = helper_check_hermite(H_t_Kp,1e-8);
    if hem1 == 0 || hem2 == 0
...
function [H_b_K, H_b_Kp] = construct_bilayer_effective_Ham(on_sites, gamma0, sqrt2_gamma1, sqrt2_gamma3, sqrt2_gamma4, akx, aky)
    ...
    Im = 1i;
    f_K_eff = (-akx + Im * aky);
    x0 = sqrt(3) / 2 * gamma0 * f_K_eff;
    x3 = sqrt(3) / 2 * sqrt2_gamma3 * f_K_eff;
    x4 = sqrt(3) / 2 * sqrt2_gamma4 * f_K_eff;
    ...
    hem1 = helper_check_hermite(H_b_K, 1e-8);
    hem2 = helper_check_hermite(H_b_Kp, 1e-8);
    if hem1 == 0 || hem2 == 0
