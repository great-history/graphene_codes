...
density_matrix_up_temp = zeros(3);
% density_matrix_down_temp = 1 / 3 * ones(3);

%   H_hf_down =
% 
%    1.0e-03 *
% 
%  -0.208908351829005 + 0.000000000000000i -0.234545829074911 - 0.036313402174444i  0.199693663416467 + 0.046848614039602i
%  -0.234545829074911 + 0.036313402174444i -0.142665435401831 + 0.000000000000000i  0.207470958091626 - 0.018226374601945i
%   0.199693663416467 - 0.046848614039602i  0.207470958091626 + 0.018226374601945i -0.029595649237020 + 0.000000000000000i
...
for ii = 1:num_steps
    ...
    Delta_mid = get_delta_mid_method2(density_matrix_down_temp, density_matrix_up_temp, transform_mat_cell, LL_index_max);
    ...
    % [H_hf_up, H_hf_down] = construct_ABA_trilayer_H_hf_T2(density_matrix_up_temp, density_matrix_down_temp, S_tensor, R_mat, ...
    %                                                       T2_eigvals_list, E_zeeman, E_H, Delta_mid, E_exchange);
    H_hf_down = construct_ABA_trilayer_H_hf_T2_spin(density_matrix_down_temp, S_tensor, R_mat, ...
                                                    T2_eigvals_list, E_zeeman, E_H, Delta_mid, E_exchange);
    ...
    density_matrix_down_new = eigvecs_down_temp(:, index_list(1)) * eigvecs_down_temp(:, index_list(1))' + ...
                                eigvecs_down_temp(:, index_list(2)) * eigvecs_down_temp(:, index_list(2))';
    
    H_hf_down
    eigvecs_down_temp
    density_matrix_down_temp
    
    error_matrix = abs(density_matrix_down_temp - density_matrix_down_new);
    b = sum(sum(abs(density_matrix_down_temp).^2));
    ...
    % damp_index = round(ii / 200);
    % density_matrix_down_temp = damp_part_list(damp_index + 1) * density_matrix_down_new + (1 - damp_part_list(damp_index + 1)) * density_matrix_down_temp;
    density_matrix_down_temp = damp_part * density_matrix_down_new + (1 - damp_part) * density_matrix_down_temp;
end
plot(error_list)
...
%     wave_packet_mesh = get_localized_wave_packet(xx_mesh, yy_mesh, dims_x, dims_y, n);
%     subplot(1,2,1)
%     im1 = imagesc(x_list, y_list, abs(wave_packet_mesh));
%     colorbar
%     subplot(1,2,2)
%     im2 = imagesc(x_list, y_list, imag(wave_packet_mesh));
%     colorbar
% end