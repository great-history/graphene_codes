...
% u的作用：high LL中电荷主要集中在第一层(u > 0)或第二层(u < 0)，而LLL中电荷主要集中在第二层(u > 0)或第一层(u <
...
for B_index = 1:B_steps
    ...
    [rho_layer1_K, rho_layer2_K] = get_layer_density_two_bands(eigvec_HK_now(:, LL_index_cutoff - 3:LL_index_cutoff), 1, LL_index_cutoff);
    [rho_layer1_Kp, rho_layer2_Kp] = get_layer_density_two_bands(eigvec_HKp_now(:, LL_index_cutoff - 3:LL_index_cutoff), -1, LL_index_cutoff);
    rho_layer1 = rho_layer1_K + rho_layer1_Kp;
    rho_layer2 = rho_layer2_K + rho_layer2_Kp;
    disp("LLL在第一层上的电荷密度")
    rho_layer1
...
fig1 = plot_LLs_weights(eigvec_HK_now(:,LL_index_cutoff - 3 :LL_index_cutoff + 3));
...
u_factor = d_interlayer / (epsilon_0 * 10^(-9)); 
...
u_internal =  u_factor * electric_internal;
