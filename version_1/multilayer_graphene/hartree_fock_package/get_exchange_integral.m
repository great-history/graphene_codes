 function X_nm = get_exchange_integral(n1, n2, m1, m2, d_interlayer)
    ...
    format long
    if ~(d_interlayer == 0) % 计算interlayer exchange integral
        % disp(varargin)
        ...
        X_nm = integral(func, 0, Inf, 'RelTol', 1e-7);
...
%     
%     format long
%     if nargin >= 5 && ~(varargin{1} == 0) % 计算interlayer exchange integral
%         % disp(varargin)
%         d_interlayer = varargin{1}; % 层间距离，已经被无量纲化了(除以magnetic length)，d / lb
%         func = @(y) exp(-sqrt(2) * d_interlayer .* sqrt(y)) .* exp(-y) .* (y.^coeff) .* laguerreL(n_min, nd, y) .* laguerreL(m_min, md, y);
%         % 当LL index > 10时, RelTol不能设为1e-10,而应该设到1e-7左右才行
