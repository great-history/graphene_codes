...
% 因此构造S矩阵只需考虑n = 0,1,2这三个LL之间的层内或层间的交换相互作用：在PRB 85，165139（2021）TABLE I中已经给出了在任意磁场下的层内交换相互作用和B=20T/30T下的层间交换相互作用    
function exchange_integrals_cell = get_all_exchange_integrals(LL_index_upper_bound, d_interlayer_list, mag_length)
    % d_interlayer_list存放的是最近邻，次紧邻的层间距 ； LL_index_upper_bound是指LL_index的上限
    % LL_index_min = 0 ; LL_index_max = 2
    % 下面就是构造出这样的S矩阵
    format long
    % 构造exchange integral
    tic
    num_LL = LL_index_upper_bound + 1;
    %% 对(n1,n2)进行编码
    indice_list = zeros(num_LL * num_LL, 3); % 将(n1,n2)按照三进制进行编码
    for ii = 0:(num_LL - 1)
        num1 = ii * num_LL;c
        for jj = 0:(num_LL - 1)
            num2 = jj * 1;
    ...
    %% 计算层间和层内的交换相互作用
    exchange_integrals_cell = cell(num_LL * num_LL, 1 + length(d_interlayer_list)); % 其实只需要三个指标即可，因为 X_{n1 n2, m1 m2}不为零需要满足条件：
