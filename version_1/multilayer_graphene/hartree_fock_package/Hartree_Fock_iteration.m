%% 误差函数
function [error, density_matrix_up_temp, density_matrix_down_temp] = Hartree_Fock_iteration(H_hf_up, H_hf_down, density_matrix_up_last, density_matrix_down_last, ...
                                                                                            filling_factor, dims)
    ...
    % 计算误差（check:应该没问题）
    %     A = abs(density_matrix_up_temp - density_matrix_up_last) + abs(density_matrix_down_temp - density_matrix_down_last);
    %     B = abs(density_matrix_up_last) + abs(density_matrix_down_last);
    %     error_matrix = A ./ (B + ones(dims));
