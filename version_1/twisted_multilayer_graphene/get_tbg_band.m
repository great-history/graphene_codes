...
% ld_corner = [-sqrt(3)/2, -1];
% width = 2 * sqrt(3); 
% np_width = 40;
% height = 3;
% np_height = 30;
% [akx_mesh, aky_mesh, k_K_list] = brillouin_rectangle(ld_corner, width, np_width, height, np_height, ak_norm);
% akx_array = transpose(akx_mesh(:));
% aky_array = transpose(akx_mesh(:));
% num_k = size(akx_array, 2);
...
%         eig_vecs_K_low_ene(i, :, :) = eig_vecs_K_low_ene_last;
    ...
    % % valley Kp
    [eigvecs_Kp, eigvals] = eig(H_tbg_Kp);
...
flag_plot_2d = true;
flag_plot_all_bands = true;
flag_plot_low_ene = false;
...
if flag_plot_2d
    if flag_plot_all_bands
        ene_shift = (eig_vals_K_low_ene(1) + eig_vals_K_low_ene(2)) / 2; % 将低能的K简并点平移到零能处

        figure
        plot(ak_len_array, eig_vals_K, 'b-','LineWidth', 2)
        hold on;
        plot(ak_len_array, eig_vals_Kp, 'r--','LineWidth', 1.5)
